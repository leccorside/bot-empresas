...
import { createWebsiteAnalysisIntent, prisma, readProviderBudget } from '@prospector/database';
import { deadLetterQueue, enqueueInsightBatch, enqueueRun, enqueueWebsiteAnalysis, insightBatchQueue, prospectingQueue, campaignQueue, removeDeadLetter, websiteAnalysisQueue } from '@prospector/queues';
...
import { directorySize, emptyQueueCounts, geographicCoverage, mergeQueueCounts, normalizeQueueCounts, verifiedBackupHealth } from './operations';

@Injectable()
export class ApiService {
  async health(){
    const started=Date.now();let database='ONLINE',redis='ONLINE',heartbeats:any[]=[];let databaseMs=0,redisMs=0;let prospecting=emptyQueueCounts(),websiteAnalysis=emptyQueueCounts(),campaigns=emptyQueueCounts(),deadLetter=emptyQueueCounts();
    ...
    const queues=[prospectingQueue(),websiteAnalysisQueue(),campaignQueue(),deadLetterQueue()];const redisStarted=Date.now();try{const [prospectingRaw,websiteRaw,campaignRaw,deadLetterRaw]=await Promise.all(queues.map(queue=>queue.getJobCounts('waiting','active','failed','delayed','paused')));prospecting=normalizeQueueCounts(prospectingRaw);websiteAnalysis=normalizeQueueCounts(websiteRaw);campaigns=normalizeQueueCounts(campaignRaw);deadLetter=normalizeQueueCounts(deadLetterRaw)}catch{redis='OFFLINE'}finally{await Promise.all(queues.map(queue=>queue.close().catch(()=>{})))}redisMs=Date.now()-redisStarted;
    const backup=await verifiedBackupHealth(process.env.BACKUPS_DIR??'/storage/backups',Number(process.env.BACKUP_MAX_AGE_HOURS??26));
    const rows=Object.fromEntries(heartbeats.map(row=>[row.key,row.value]));const services={api:'ONLINE',database,redis,worker:heartbeatStatus(rows['service:worker'],45000),websiteAnalyzer:heartbeatStatus(rows['service:website-analyzer'],45000),scheduler:heartbeatStatus(rows['service:scheduler'],90000),recovery:heartbeatStatus(rows['service:recovery'],90000),reconciliation:heartbeatStatus(rows['service:reconciliation'],90000),websiteRefresh:heartbeatStatus(rows['service:website-refresh'],90000),backup:backup.status};
    return{status:Object.values(services).every(value=>value==='ONLINE')?'ok':'degraded',services,queues:{prospecting,websiteAnalysis,campaigns,deadLetter,total:mergeQueueCounts(prospecting,websiteAnalysis,campaigns)},backup,latencyMs:{database:databaseMs,redis:redisMs,total:Date.now()-started},uptimeSeconds:Math.floor(process.uptime()),timestamp:new Date().toISOString()}
  }
  ...
  async dashboard(){
    ...
    const [coverageCells,googlePlacesBudget]=await Promise.all([prisma.searchCell.findMany({select:{status:true,resultsFound:true}}),readProviderBudget('GOOGLE_PLACES',Number(process.env.GOOGLE_PLACES_DAILY_REQUEST_LIMIT??1000))]);
    return{metrics:{businesses,withWebsite,withoutWebsite:businesses-withWebsite,withPhone,withoutPhone:businesses-withPhone,whatsapp,noReviews,high},runs,schedules,jobs:Object.fromEntries(jobs.map(job=>[job.state,job._count])),queues:health.queues,services:health.services,backup:health.backup,latencyMs:health.latencyMs,uptimeSeconds:health.uptimeSeconds,storage:{databaseBytes:Number(databaseSize[0]?.bytes??0),exportsBytes,logsBytes,exportFiles},coverage:geographicCoverage(coverageCells,Number(process.env.GOOGLE_PLACES_MAX_PAGES_PER_CELL??3)),providerBudgets:{googlePlaces:googlePlacesBudget},healthCheckedAt:health.timestamp,settings:{dryRun:process.env.DRY_RUN!=='false',...(automation?.value as object??{})}}
  }
  ...
  async cancelInsightBatch(id:string){const batch=await prisma.insightBatch.findUnique({where:{id}});if(!batch)throw new NotFoundException('Lote não encontrado');if(['COMPLETED','CANCELLED','FAILED'].includes(batch.status))return batch;const queue=insightBatchQueue();try{await(await queue.getJob(`insight-batch-${id}`))?.remove()}catch{}finally{await queue.close()}const ids=(batch.filters as any)?.businessIds;const [cancelled]=await prisma.$transaction([prisma.insightBatch.update({where:{id},data:{status:'CANCELLED',completedAt:new Date()}}),prisma.business.updateMany({where:{id:{in:Array.isArray(ids)?ids:[]}},data:{aiInsightQueuedAt:null}}),prisma.jobRecord.updateMany({where:{idempotencyKey:`insight-batch:${id}`},data:{state:'CANCELLED',completedAt:new Date()}})]);return cancelled}
  ...
  async jobAction(id:string,action:string){
    const job=await prisma.jobRecord.findUnique({where:{id}});if(!job)throw new NotFoundException();
    if(action==='retry'){
      if(job.runId){await prisma.$transaction([prisma.prospectingRun.update({where:{id:job.runId},data:{status:'RECOVERING',errorMessage:null,finishedAt:null}}),prisma.jobRecord.update({where:{id},data:{state:'RECOVERING',errorMessage:null,completedAt:null}})]);await enqueueRun(job.runId)}
      else if(job.queue==='website-analysis'){const analysisId=String((job.payload as any)?.analysisId??'');const analysis=await prisma.websiteAnalysis.findUnique({where:{id:analysisId}});if(!analysis)throw new NotFoundException('Análise não encontrada');await prisma.$transaction([prisma.websiteAnalysis.update({where:{id:analysis.id},data:{status:'RECOVERING',errorMessage:null,completedAt:null}}),prisma.jobRecord.update({where:{id},data:{state:'RECOVERING',errorMessage:null,completedAt:null}})]);await enqueueWebsiteAnalysis(analysis.id)}
      else if(job.queue==='insight-batch'){const batchId=String((job.payload as any)?.batchId??'');await prisma.$transaction([prisma.insightBatch.update({where:{id:batchId},data:{status:'RECOVERING',errorMessage:null,completedAt:null}}),prisma.jobRecord.update({where:{id},data:{state:'RECOVERING',errorMessage:null,completedAt:null}})]);await enqueueInsightBatch(batchId)}
      else if(job.queue==='campaign'){const campaignId=String((job.payload as any)?.campaignId??'');const queue=campaignQueue();try{const queued=await queue.getJob(job.bullJobId??`campaign-${campaignId}`);if(queued&&await queued.getState()==='failed')await queued.retry();else if(!queued)await queue.add('send-campaign',{campaignId},{jobId:`campaign-${campaignId}`})}finally{await queue.close()}await prisma.$transaction([prisma.campaign.update({where:{id:campaignId},data:{status:'SCHEDULED',finishedAt:null}}),prisma.jobRecord.update({where:{id},data:{state:'RECOVERING',errorMessage:null,completedAt:null}})])}
      else throw new BadRequestException('Fila sem reprocessamento automático');
      if(job.bullJobId)await removeDeadLetter(job.queue,job.bullJobId);return{ok:true};
    }
    if(action==='archive'){if(job.bullJobId)await removeDeadLetter(job.queue,job.bullJobId);return prisma.jobRecord.update({where:{id},data:{state:'CANCELLED',completedAt:new Date()}})}
    if(action==='cancel'){if(job.queue==='website-analysis'){const analysisId=String((job.payload as any)?.analysisId??'');if(analysisId)await prisma.websiteAnalysis.updateMany({where:{id:analysisId},data:{status:'CANCELLED'}});const queue=websiteAnalysisQueue();try{await (await queue.getJob(`website-analysis-${analysisId}`))?.remove()}catch{}finally{await queue.close()}}return prisma.jobRecord.update({where:{id},data:{state:'CANCELLED'}})}throw new BadRequestException('Ação inválida')}
  ...
  async settings(){const [row,configRow]=await Promise.all([prisma.systemSetting.findUnique({where:{key:'automation'}}),prisma.systemSetting.findUnique({where:{key:'autopilotConfig'}})]);return{dryRun:process.env.DRY_RUN!=='false',autoSendCampaigns:process.env.AUTO_SEND_CAMPAIGNS==='true',autoGenerateInsights:process.env.AUTO_GENERATE_INSIGHTS==='true',aiInsightRefreshDays:Number(process.env.AI_INSIGHT_REFRESH_DAYS??30),automation:row?.value??{paused:false,autopilot:false},autopilotConfig:parseAutopilotConfig(configRow?.value)}}
