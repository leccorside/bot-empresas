      ...
      while(!done){
        ...
        for(const item of found.results){
          ...
          await prisma.$transaction(async tx=>{
            ...
            await tx.businessSnapshot.create({data:{businessId:business.id,rating:item.rating,reviewsCount:item.reviewsCount,website:item.website,phone:item.phone}});
            await tx.discoveryEvent.upsert({where:{runId_businessId:{runId:run.id,businessId:business.id}},update:{},create:{runId:run.id,businessId:business.id,cellId:cell.id,wasNew:!existing}});
            await tx.processingCheckpoint.upsert({where:{runId_stage_entityType_entityId:{runId:run.id,stage:'DISCOVERY',entityType:'CELL',entityId:cell.id}},update:{page:page+1,processedItems:{increment:1},status:'RUNNING',metadata:{nextPageToken:found.nextPageToken??null}},create:{runId:run.id,stage:'DISCOVERY',entityType:'CELL',entityId:cell.id,page:page+1,processedItems:1,status:'RUNNING',metadata:{nextPageToken:found.nextPageToken??null}}});
          });
        }
