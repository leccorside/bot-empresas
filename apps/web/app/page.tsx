...
const services = [['API', 'api'], ['POSTGRES', 'database'], ['REDIS', 'redis'], ['WORKER', 'worker'], ['WEBSITE ANALYZER', 'websiteAnalyzer'], ['SCHEDULER', 'scheduler'], ['RECOVERY', 'recovery'], ['RECONCILIATION', 'reconciliation'], ['REFRESH INCREMENTAL', 'websiteRefresh'], ['BACKUP VERIFICADO', 'backup']];
...
export default function Dashboard() {
  ...
  return <Shell title="Dashboard operacional" subtitle="PostgreSQL como fonte de verdade e BullMQ como mecanismo de execução" badges={<>
    ...
    <div className="grid operationalGrid">
      ...
      <section className="card"><h2 className="sectionTitle">Filas BullMQ em tempo real</h2><table className="table compactTable"><thead><tr><th>Fila</th><th>Espera</th><th>Ativos</th><th>Delay</th><th>Falhos</th></tr></thead><tbody>{[['Prospecção', data?.queues?.prospecting], ['Website Analyzer', data?.queues?.websiteAnalysis], ['Campanhas', data?.queues?.campaigns], ['Dead letter', data?.queues?.deadLetter]].map(([label, queue]: any) => <tr key={label}><td>{label}</td><td>{queue?.waiting ?? '—'}</td><td>{queue?.active ?? '—'}</td><td>{queue?.delayed ?? '—'}</td><td>{queue?.failed ?? '—'}</td></tr>)}</tbody></table><div className="durableJobs"><span>Estado durável</span><span>WAITING <b>{data?.jobs?.WAITING ?? 0}</b></span><span>ACTIVE <b>{data?.jobs?.ACTIVE ?? 0}</b></span><span>FAILED <b>{data?.jobs?.FAILED ?? 0}</b></span></div></section>
      <section className="card"><h2 className="sectionTitle">Armazenamento persistente</h2><div className="storageMetrics"><div><span>PostgreSQL</span><b>{formatBytes(data?.storage?.databaseBytes)}</b></div><div><span>Exportações</span><b>{formatBytes(data?.storage?.exportsBytes)}</b><small>{data?.storage?.exportFiles ?? 0} arquivos catalogados</small></div><div><span>Logs</span><b>{formatBytes(data?.storage?.logsBytes)}</b></div><div><span>Backup verificado</span><b>{formatBytes(data?.backup?.sizeBytes)}</b><small>{data?.backup?.archive??'não disponível'}</small></div></div></section>
    </div>

    <div className="grid dashboardTables">
      <section className="card"><h2 className="sectionTitle">Cobertura geográfica</h2><div className="storageMetrics"><div><span>Cobertura concluída</span><b>{data?.coverage?.percent ?? 0}%</b><small>{data?.coverage?.completed ?? 0}/{data?.coverage?.total ?? 0} células</small></div><div><span>Resultados por célula</span><b>{data?.coverage?.averageResultsPerCell ?? 0}</b><small>{data?.coverage?.results ?? 0} resultados brutos</small></div><div><span>Células vazias</span><b>{data?.coverage?.empty ?? 0}</b><small>sem resultados</small></div><div><span>Células saturadas</span><b>{data?.coverage?.saturated ?? 0}</b><small>atingiram o limite de páginas</small></div></div></section>
      <section className="card"><h2 className="sectionTitle">Orçamento Google Places</h2><div className="budgetGauge"><div><b>{data?.providerBudgets?.googlePlaces?.used ?? 0}</b> / {data?.providerBudgets?.googlePlaces?.limit ?? '—'} solicitações hoje</div><div className="budgetTrack"><i style={{width:`${Math.min(100,data?.providerBudgets?.googlePlaces?.percent ?? 0)}%`}} /></div><div className="sub">{data?.providerBudgets?.googlePlaces?.remaining ?? '—'} restantes · alerta a partir de 80%</div><Status value={data?.providerBudgets?.googlePlaces?.status ?? 'CHECKING'} /></div></section>
    </div>
