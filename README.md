...
Para gerar insights automaticamente, habilite `AUTO_GENERATE_INSIGHTS=true`. O scheduler cria lotes persistentes de até `AI_AUTO_INSIGHT_BATCH_SIZE` empresas sem insight ou com insight mais antigo que `AI_INSIGHT_REFRESH_DAYS`. A reserva `aiInsightQueuedAt` evita duplicidade e expira após `AI_INSIGHT_LEASE_MINUTES`; a fila é reconstruída pelo recovery após perda do Redis. A opção vem desligada para impedir consumo inesperado de APIs pagas.
